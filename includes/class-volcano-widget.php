...
/** @define "VOLCANO_ADDONS_WIDGETS_PATH" "./../widgets/" */


/**
 * Class Volcano_Widget
 * @package VolcanoAddons
 */
class Volcano_Widget extends Widget_Base {
	...
	public function get_name() {
		if ( null === $this->name ) {
			$called_class = str_replace(
				[
					'VolcanoAddons\\Widgets\\',
				],
				'',
				get_called_class()
			);
			$called_class = str_replace( 'VolcanoAddons_Style_', '', $called_class );
			$called_class = str_replace( '_', '-', $called_class );
			$this->name   = 'volcano-' . strtolower( $called_class );
		}
	...
	protected function get_base_id() {
		if ( null === $this->base_id ) {
			$this->base_id = str_replace( 'volcano-', '', $this->get_name() );
		}
	...
	protected function add_render_attributes() {
		...
		if ( apply_filters( "volcano_addons/widgets/{$this->get_base_id()}/use_style_type", $this->use_style_type ) ) {
	...
	protected function plugin_dependency_alert( $dependencies ) {
		...
		foreach ( $dependencies as $dependency ) {
			...
			foreach ( $dependency['plugins'] as $plugin ) {
				...
				if ( volcano_addons_is_plugin_active( $plugin['path'] ) ) {
					continue;
				}
				...
				/* translators: 1. Required Plugin Name. 2. Plugin installation/activation URL */
					__( '<strong>%1$s</strong> is not installed/activated on your site. Click <a href="%2$s" target="_blank" rel="noopener noreferrer">here</a> to install/activate <strong>%1$s</strong> first.', 'volcano-addons' ),
					$plugin['name'],
					esc_url( volcano_addons_plugin_install_url( $plugin['slug'] ) )
				);
			}
			...
			if ( ! empty( $messages ) ) {
				...
				$control_id = 'volcano_addons_dependency_alert_' . $this->get_style_id() . '_' . $control_id;
	...
	protected function get_style_slug( $settings ) {
		...
		$slug = isset( $settings[ 'volcano_addons_' . $key ] ) ? $settings[ 'volcano_addons_' . $key ] : '';
		if ( ! $slug ) {
	...
	protected function render_icon( $key, $default = '', $settings = [], $tag = 'i', $compatibility_key = 'volcano-addons' ) {
		if ( empty( $settings ) ) {
			$settings = $this->get_settings_for_display();
		}
		...
		if ( $has_icon ) {
			if ( $is_new || $migrated ) {
				...
				if ( $default ) {
					?>
					<<?php volcano_addons_tag_name( $tag ); ?> class="<?php echo esc_attr( $default ); ?>" aria-hidden="true"></<?php volcano_addons_tag_name( $tag ); ?>>
					<?php
				}
			}
		}
	...
	private function is_plugin_file( $file_path ) {
		return (
			0 === strpos( $file_path, untrailingslashit( VOLCANO_ADDONS_PATH ) )
		);
	}
	 ...
	 * @return array
	 */
	protected function list_templates( $slug, $path = VOLCANO_ADDONS_WIDGETS_PATH ) {
		return [
	...
	protected function list_controller( $slug = '', $path = VOLCANO_ADDONS_WIDGETS_PATH ) {
		$controller = [
			sprintf(
	 ...
	 * @return array
	 */
	protected function list_content_templates( $slug, $path = VOLCANO_ADDONS_WIDGETS_PATH ) {
		return [
			// @XXX if we introduce template override from the theme, then it would be added on the top of main template.
			sprintf(
